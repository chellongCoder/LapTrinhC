...
void VeDuongDua_1() {
    ...
    for (i=0; i<30; i++) {
        ...
        map[i][29]='|'; //t?o du?ng bên ph?i
        if(i%2==0) {
            map[i][14]='|'; // t?o v?ch du?ng
        }
        for (j=1; j<29&&j!=14; j++) {
                map[i][j]=' ';
        }
...
void InDuongDua() {
    ...
    for (i=0; i<30; i++) {
        ...
        for (j=0; j<30; j++) {
			...
			else if (j==14) { //xóa lằn đường cũ đi
				TextColor(3);
				printf("%c",map[i][j]);
				map[i][j]=' ';
				TextColor(7);
...
void DiChuyenXe(int&x, int&y) {
    ...
    //di xuống
		...
		if (x>1) {
			map[x][y]=' '; //khung xe
			map[x][y-1]=' '; //thân trái xe
			map[x][y+1]=' '; //thân phai xe
			map[x+1][y-1]=' '; // bánh xe trái duoi
			map[x+1][y+1]=' ';//bánh xe phảii duới
			map[x-1][y-1]=' '; //basnh xe trái trên
			map[x-1][y+1]=' '; //bánh xe phai trên
			x--;
			VeXePlayer(x,y);
		}
    }
